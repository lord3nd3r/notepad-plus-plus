...
namespace {
// Use an unnamed namespace to protect the functions and classes from name
// conflicts
...
BracketPair FindBracketPair(Tokens &tokens) {
  const Tokens::iterator itBracket =
...
void highlightTaskMarker(StyleContext &sc, LexAccessor &styler, int activity,
                         const WordList &markerList, bool caseSensitive) {
  if (IsOperatorOrSpace(sc.chPrev) && !IsOperatorOrSpace(sc.ch) &&
      markerList.Length()) {
    std::string marker;
...
const LexillaCharacterSet setHexDigits(LexillaCharacterSet::setDigits,
                                       "ABCDEFabcdef");
const LexillaCharacterSet setOctDigits("01234567");
const LexillaCharacterSet setNoneNumeric;

class EscapeSequence {
  const LexillaCharacterSet *escapeSetValid = nullptr;
  int digitsLeft = 0;
...
public:
  ...
  void consumeDigit() noexcept { digitsLeft--; }
};

std::string GetRestOfLine(LexAccessor &styler, Sci_Position start,
                          bool allowSpace) {
...
struct PPDefinition {
  ...
  PPDefinition(Sci_Position line_, std::string_view key_,
               std::string_view value_, bool isUndef_,
               std::string_view arguments_)
...
class LinePPState {
  // Track the state of preprocessor conditionals to allow showing active and
  // inactive code in different styles. Only works up to 31 levels of
  // conditional nesting.
...
public:
  ...
  [[nodiscard]] bool IsActive() const noexcept { return state == 0; }
  [[nodiscard]] bool IsInactive() const noexcept { return state != 0; }
...
// Currently one entry per line but could become sparse with just one entry per
// preprocessor line.
class PPStates {
...
// Parse a macro definition, either from a #define line in a file or from
// keywords. Either an object macro <NAME> <VALUE> or a function macro
// <NAME>(<ARGUMENTS>) <VALUE>. VALUE is optional and is treated as "1" if
// missing. #define ALLOW_PRINT #define VERSION 37 #define VER(a,b) a*10+b
...
constexpr Definition ParseDefine(std::string_view definition,
  ...
  if (afterName != std::string_view::npos) {
    ...
    if (definition.at(afterName) == '(') {
      ...
      if (closeBracket != std::string_view::npos) {
        ...
        if (!definition.empty() &&
            (endName.find(definition.front()) != std::string_view::npos)) {
...
struct OptionSetCPP : public OptionSet<OptionsCPP> {
  OptionSetCPP() {
    DefineProperty("styling.within.preprocessor",
                   &OptionsCPP::stylingWithinPreprocessor,
                   "For C++ code, determines whether all preprocessor code is "
                   "styled in the "
                   "preprocessor style (0, the default) or only from the "
                   "initial # to the end "
                   "of the command word(1).");

    DefineProperty("lexer.cpp.allow.dollars",
                   &OptionsCPP::identifiersAllowDollars,
                   "Set to 0 to disallow the '$' character in identifiers with "
                   "the cpp lexer.");

    DefineProperty("lexer.cpp.track.preprocessor",
                   &OptionsCPP::trackPreprocessor,
                   "Set to 1 to interpret #if/#else/#endif to grey out code "
                   "that is not active.");

    DefineProperty(
        "lexer.cpp.update.preprocessor", &OptionsCPP::updatePreprocessor,
    ...
    DefineProperty("lexer.cpp.triplequoted.strings",
                   &OptionsCPP::triplequotedStrings,
                   "Set to 1 to enable highlighting of triple-quoted strings.");

    DefineProperty("lexer.cpp.hashquoted.strings",
                   &OptionsCPP::hashquotedStrings,
                   "Set to 1 to enable highlighting of hash-quoted strings.");

    DefineProperty("lexer.cpp.backquoted.strings",
                   &OptionsCPP::backQuotedStrings,
    ...
    DefineProperty(
        "lexer.cpp.escape.sequence", &OptionsCPP::escapeSequence,
    ...
    DefineProperty("fold.comment", &OptionsCPP::foldComment,
                   "This option enables folding multi-line comments and "
                   "explicit fold points when using the C++ lexer. "
                   "Explicit fold points allows adding extra folding by "
                   "placing a //{ comment at the start and a //} "
                   "at the end of a section that should fold.");

    DefineProperty("fold.cpp.comment.multiline",
                   &OptionsCPP::foldCommentMultiline,
                   "Set this property to 0 to disable folding multi-line "
                   "comments when fold.comment=1.");

    DefineProperty("fold.cpp.comment.explicit",
                   &OptionsCPP::foldCommentExplicit,
                   "Set this property to 0 to disable folding explicit fold "
                   "points when fold.comment=1.");

    DefineProperty("fold.cpp.explicit.start", &OptionsCPP::foldExplicitStart,
                   "The string to use for explicit fold start points, "
                   "replacing the standard //{.");

    DefineProperty("fold.cpp.explicit.end", &OptionsCPP::foldExplicitEnd,
                   "The string to use for explicit fold end points, replacing "
                   "the standard //}.");

    DefineProperty("fold.cpp.explicit.anywhere",
                   &OptionsCPP::foldExplicitAnywhere,
                   "Set this property to 1 to enable explicit fold points "
                   "anywhere, not just in line comments.");

    DefineProperty("fold.cpp.preprocessor.at.else",
                   &OptionsCPP::foldPreprocessorAtElse,
                   "This option enables folding on a preprocessor #else or "
                   "#endif line of an #if statement.");

    DefineProperty(
        "fold.preprocessor", &OptionsCPP::foldPreprocessor,
        "This option enables folding preprocessor directives when using the "
        "C++ lexer. "
    ...
    DefineProperty("fold.at.else", &OptionsCPP::foldAtElse,
                   "This option enables C++ folding on a \"} else {\" line of "
                   "an if statement.");
...
const LexicalClass lexicalClasses[] = {
    ...
    "comment preprocessor documentation",
...
class LexerCPP : public ILexer5 {
  ...
  LexillaCharacterSet setAddOp;
  LexillaCharacterSet setMultOp;
  ...
  struct SymbolValue {
    ...
    SymbolValue(std::string_view value_, std::string_view arguments_)
    ...
    [[nodiscard]] bool IsMacro() const noexcept { return !arguments.empty(); }
  };
  ...
  SubStyles subStyles{styleSubable, SubStylesFirst, SubStylesAvailable,
                      inactiveFlag};
...
public:
  explicit LexerCPP(bool caseSensitive_)
      : caseSensitive(caseSensitive_),
        setWord(LexillaCharacterSet::setAlphaNum, "._", true),
        setNegationOp("!"), setAddOp("+-"), setMultOp("*/%"), setRelOp("=!<>"),
        setLogicalOp("|&") {}
  ...
  int SCI_METHOD NamedStyles() override {
    return std::max(subStyles.LastAllocated() + 1, sizeLexicalClasses) +
           inactiveFlag;
  }
  ...
  static ILexer5 *LexerFactoryCPP() { return new LexerCPP(true); }
  static ILexer5 *LexerFactoryCPPInsensitive() { return new LexerCPP(false); }
  constexpr static int MaskActive(int style) noexcept {
    return style & ~inactiveFlag;
  }
  void EvaluateTokens(Tokens &tokens,
...
Sci_Position SCI_METHOD LexerCPP::PropertySet(const char *key,
                                              const char *val) {
  if (osCPP.PropertySet(&options, key, val)) {
    if (strcmp(key, "lexer.cpp.allow.dollars") == 0) {
      setWord =
          LexillaCharacterSet(LexillaCharacterSet::setAlphaNum, "._", true);
      ...
      if (n == 4) {
        ...
        for (int nDefinition = 0; nDefinition < ppDefinitions.Length();
          ...
          const Definition def =
              ParseDefine(ppDefinitions.WordAt(nDefinition), "(=");
          preprocessorDefinitionsStart[std::string(def.name)] =
              SymbolValue(def.value, def.arguments);
        }
      }
    }
  }
...
void SCI_METHOD LexerCPP::Lex(Sci_PositionU startPos, Sci_Position length,
  ...
  const StyleContext::Transform transform =
      caseSensitive ? StyleContext::Transform::none
                    : StyleContext::Transform::lower;

  const LexillaCharacterSet setOKBeforeRE("([{=,:;!%^&*|?~+-> ");
  const LexillaCharacterSet setCouldBePostOp("+-");

  const LexillaCharacterSet setDoxygen(LexillaCharacterSet::setAlpha,
                                       "$@\\&<>#{}[]");

  setWordStart = LexillaCharacterSet(LexillaCharacterSet::setAlpha, "_", true);

  const LexillaCharacterSet setInvalidRawFirst(" )\\\t\v\f\n");
  ...
  const std::vector<PPDefinition>::iterator itInvalid =
      std::find_if(ppDefineHistory.begin(), ppDefineHistory.end(),
                   [lineCurrent](const PPDefinition &p) noexcept {
  ...
  for (const PPDefinition &ppDef : ppDefineHistory) {
    if (ppDef.isUndef)
      ...
      preprocessorDefinitions[ppDef.key] =
          SymbolValue(ppDef.value, ppDef.arguments);
  }

  std::string rawStringTerminator =
      rawStringTerminators.ValueAt(lineCurrent - 1);
  ...
  for (; sc.More();) {
    ...
    if (sc.atLineStart) {
      ...
      // Inside inactive preprocessor declaration, state will be reset anyway at
      // the end of this block.
      if ((sc.state == SCE_C_STRING) || (sc.state == SCE_C_CHARACTER)) {
        // Prevent SCE_C_STRINGEOL from leaking back to previous line which
        // ends with a line continuation by locking in the state up to this
        // position.
      ...
      if (sc.atLineStart || sc.atLineEnd || !setWord.Contains(sc.ch) ||
        ...
        if (literalString || sc.ch == '\'') {
          ...
          if (valid) {
            if (literalString) {
              if (raw) {
                // Set the style of the string prefix to SCE_C_STRINGRAW but
                // then change to SCE_C_DEFAULT as that allows the raw string
                // start code to run.
    ...
    case SCE_C_COMMENTDOCKEYWORD:
      if ((styleBeforeDCKeyword == SCE_C_COMMENTDOC) && sc.Match('*', '/')) {
        ...
        if (!(IsASpace(sc.ch) || (sc.ch == 0))) {
          ...
          if (!keywords3.InList(currentSuffix) &&
              !keywords3.InList(currentText)) {
            const int subStyleCDKW =
                classifierDocKeyWords.ValueFor(currentSuffix);
            if (subStyleCDKW >= 0) {
        ...
        if (!keywords3.InList(currentText)) {
          const int subStyleCDKW =
              classifierDocKeyWords.ValueFor(currentText.substr(1));
          if (subStyleCDKW >= 0) {
            sc.ChangeState(subStyleCDKW | activitySet);
          } else {
            sc.ChangeState(SCE_C_COMMENTDOCKEYWORDERROR | activitySet);
    ...
    case SCE_C_STRINGRAW:
      ...
      } else if (options.backQuotedStrings ==
                 BackQuotedString::TemplateLiteral) {
        if (sc.ch == '\\') {
          if (options.escapeSequence) {
            escapeSeq.resetEscapeState(sc.state, sc.chNext);
            sc.SetState(SCE_C_ESCAPESEQUENCE | activitySet);
    ...
    if (MaskActive(sc.state) == SCE_C_DEFAULT) {
      if (sc.Match('@', '\"')) {
        ...
        sc.Forward();
      } else if ((options.backQuotedStrings != BackQuotedString::None) &&
                 sc.Match('`')) {
        ...
        if (lastWordWasUUID) {
          ...
          sc.SetState(SCE_C_NUMBER | activitySet);
                 ...
                 (setWordStart.Contains(sc.ch) || (sc.ch == '@'))) {
        if (lastWordWasUUID) {
          sc.SetState(SCE_C_UUID | activitySet);
          lastWordWasUUID = false;
        } else {
        ...
        if (sc.Match("/**") ||
            sc.Match("/*!")) { // Support of Qt/Doxygen doc. style
        ...
        if (sc.chPrev == 'R') {
          styler.Flush();
          if (MaskActive(styler.StyleAt(sc.currentPos - 1)) ==
              SCE_C_STRINGRAW) {
            sc.SetState(SCE_C_STRINGRAW | activitySet);
            rawStringTerminator = ")";
        ...
        if (sc.Match("include")) {
          ...
          if (options.trackPreprocessor && IsAlphaNumeric(sc.ch)) {
            // If #if is nested too deeply (>31 levels) the active/inactive
            // appearance will stop reflecting the code.
            if (sc.Match("ifdef") || sc.Match("ifndef")) {
              const bool isIfDef = sc.Match("ifdef");
              const int startRest = isIfDef ? 5 : 6;
              const std::string restOfLine =
                  GetRestOfLine(styler, sc.currentPos + startRest + 1, false);
              const bool foundDef = preprocessorDefinitions.find(restOfLine) !=
              ...
              // #else is shown as active if either preceding or following
              // section is active as that means that it contributed to the
              // result.
              if (preproc.ValidLevel()) {
                // If #else has no corresponding #if then take no action as
                // invalid
                if (!preproc.CurrentIfTaken()) {
                  ...
                  preproc.InvertCurrentLevel();
                  activitySet = preproc.ActiveState();
                  // Continue to show "else" as active as it ends active
                  // section.
                }
              }
            } else if (sc.Match("elif")) {
              // Ensure only one chosen out of #if .. #elif .. #elif .. #else ..
              // #endif #elif is shown as active if either preceding or
              // following section is active as that means that it contributed
              // to the result.
              if (preproc.ValidLevel()) {
                if (!preproc.CurrentIfTaken()) {
                  // Inactive, if expression true then may become active if
                  // parent scope active
                  assert(sc.state == (SCE_C_PREPROCESSOR | inactiveFlag));
                  // Similar to #if
                  const std::string restOfLine =
                  ...
                  preproc.InvertCurrentLevel();
                  activitySet = preproc.ActiveState();
                  // Continue to show "elif" as active as it ends active
                  // section.
                }
              }
              ...
              if (options.updatePreprocessor && preproc.IsActive()) {
                ...
                preprocessorDefinitions[std::string(def.name)] =
                    SymbolValue(def.value, def.arguments);
                ppDefineHistory.emplace_back(lineCurrent, def.name, def.value,
                                             false, def.arguments);
                definitionsChanged = true;
              }
  ...
  const bool rawStringsChanged =
      rawStringTerminators.Merge(rawSTNew, lineCurrent);
...
void SCI_METHOD LexerCPP::Fold(Sci_PositionU startPos, Sci_Position length,
  ...
  const bool userDefinedFoldMarkers =
      !options.foldExplicitStart.empty() && !options.foldExplicitEnd.empty();
  for (Sci_PositionU i = startPos; i < endPos; i++) {
    ...
    if (options.foldPreprocessor && (style == SCE_C_PREPROCESSOR)) {
      if (ch == '#') {
        ...
        if (options.foldPreprocessorAtElse &&
            (styler.Match(j, "else") || styler.Match(j, "elif"))) {
          levelMinCurrent--;
        }
      }
    }
    ...
    if (atEOL || (i == endPos - 1)) {
      ...
      const int lev = FoldLevelForCurrentNext(levelUse, levelNext) |
                      FoldLevelFlags(levelUse, levelNext,
                                     visibleChars == 0 && options.foldCompact);
      ...
      if (atEOL && (i == static_cast<Sci_PositionU>(styler.Length() - 1))) {
        ...
        styler.SetLevel(lineCurrent, FoldLevelForCurrent(levelCurrent) |
...
void LexerCPP::EvaluateTokens(Tokens &tokens,
                              const SymbolTable &preprocessorDefinitions) {
    ...
    if (tokens[i] == "defined") {
      ...
      if (tokens[i + 1] == "(") {
        if (((i + 2) < tokens.size()) && (tokens[i + 2] == ")")) {
          ...
          // defined(<identifier>)
          const SymbolTable::const_iterator it =
        ...
        // defined <identifier>
        const SymbolTable::const_iterator it =
            preprocessorDefinitions.find(tokens[i + 1]);
        if (it != preprocessorDefinitions.end()) {
          val = "1";
        }
        tokens.erase(tokens.begin() + i + 1, tokens.begin() + i + 2);
  ...
  for (size_t i = 0; (i < tokens.size()) && (iterations < maxIterations);) {
    ...
    if (setWordStart.Contains(tokens[i][0])) {
      const SymbolTable::const_iterator it =
          preprocessorDefinitions.find(tokens[i]);
      if (it != preprocessorDefinitions.end()) {
        // Tokenize value
        ...
        if (it->second.IsMacro()) {
          if ((i + 1 < tokens.size()) && (tokens.at(i + 1) == "(")) {
            ...
            macroTokens.erase(std::remove_if(macroTokens.begin(),
                                             macroTokens.end(), OnlySpaceOrTab),
                              macroTokens.end());
            ...
            for (size_t iMacro = 0; iMacro < macroTokens.size();) {
              if (setWordStart.Contains(macroTokens[iMacro][0])) {
                const std::map<std::string, std::string>::const_iterator
                    itFind = arguments.find(macroTokens[iMacro]);
                if (itFind != arguments.end()) {
                  // TODO: Possible that value will be expression so should
                  // insert tokenized form
  ...
  while (bracketPair.itBracket != tokens.end()) {
    ...
    // The insertion is done before the removal because there were failures with
    // the opposite approach
    tokens.insert(bracketPair.itBracket, inBracket.begin(), inBracket.end());

    // insert invalidated bracketPair. Use a new variable to avoid warning from
    // Coverity.
  ...
  for (int prec = precMult; prec < precLast; prec++) {
    ...
    for (size_t k = 0; (k + 2) < tokens.size();) {
      ...
      if (((prec == precMult) && setMultOp.Contains(chOp)) ||
...
extern const LexerModule lmCPP(SCLEX_CPP, LexerCPP::LexerFactoryCPP, "cpp",
                               cppWordLists);
extern const LexerModule lmCPPNoCase(SCLEX_CPPNOCASE,
