...
constexpr bool StyleForceBacktrack(int state) noexcept {
  return AnyOf(state, SCE_SH_CHARACTER, SCE_SH_STRING, SCE_SH_BACKTICKS,
               SCE_SH_HERE_Q, SCE_SH_PARAM);
}
...
const char *const bashWordListDesc[] = {"Keywords", nullptr};
...
struct OptionSetBash : public OptionSet<OptionsBash> {
  OptionSetBash() {
    ...
    DefineProperty(
        "lexer.bash.styling.inside.string", &OptionsBash::stylingInsideString,
        "Set this property to 1 to highlight shell expansions inside string.");

    DefineProperty("lexer.bash.styling.inside.backticks",
                   &OptionsBash::stylingInsideBackticks,
                   "Set this property to 1 to highlight shell expansions "
                   "inside backticks.");

    DefineProperty("lexer.bash.styling.inside.parameter",
                   &OptionsBash::stylingInsideParameter,
                   "Set this property to 1 to highlight shell expansions "
                   "inside ${} parameter expansion.");

    DefineProperty("lexer.bash.styling.inside.heredoc",
                   &OptionsBash::stylingInsideHeredoc,
                   "Set this property to 1 to highlight shell expansions "
                   "inside here document.");

    DefineProperty("lexer.bash.command.substitution",
                   &OptionsBash::commandSubstitution,
    ...
    DefineProperty("lexer.bash.nested.backticks", &OptionsBash::nestedBackticks,
                   "Set this property to 0 to disable nested backquoted "
                   "command substitution.");

    DefineProperty("lexer.bash.special.parameter",
                   &OptionsBash::specialParameter,
...
public:
  ...
  QuoteStackCls(const LexillaCharacterSet &setParamStart_) noexcept
      : setParamStart{setParamStart_} {}
  [[nodiscard]] bool Empty() const noexcept { return Current.Up == '\0'; }
  void Start(int u, QuoteStyle s, int outer, CmdState state) noexcept {
    if (Empty()) {
  ...
  void Expand(StyleContext &sc, CmdState &cmdState, bool stylingInside) {
    ...
    if (sc.ch == '{') {
      ...
      if (sc.ch == '[' || sc.chNext == '(') {
        ...
        if (stylingInside &&
            commandSubstitution >= CommandSubstitution::Inside) {
  ...
  void Escape(StyleContext &sc) {
    ...
    if (backtickLevel > 0 && nestedBackticks) {
      ...
      * when $N = n\times 2^{k + 1} + 2^k - 1$, following backtick starts inner
      substitution.
      * when $N = m\times 2^k + 2^{k - 1} - 1$ and $k > 1$, following backtick
      ends current substitution.
      */
      if (sc.chNext == '$') {
...
const LexicalClass lexicalClasses[] = {
    ...
    13,
    "SCE_SH_HERE_Q",
    "here-doc literal string",
    "Heredoc quoted string",
};

// Explicitly qualify all LexillaCharacterSet references
...
public:
  LexerBash()
      : DefaultLexer("bash", SCLEX_BASH),
        setParamStart(LexillaCharacterSet::setAlphaNum,
                      "_" BASH_SPECIAL_PARAMETER) {
    cmdDelimiter.Set("| || |& & && ; ;; ( ) { }");
  ...
  void SCI_METHOD Fold(Sci_PositionU startPos_, Sci_Position length,
                       int initStyle, IDocument *pAccess) override;
  ...
  const char *SCI_METHOD GetSubStyleBases() override { return styleSubable; }

  bool
  IsTestOperator(const char *s,
                 const LexillaCharacterSet &setSingleCharOp) const noexcept {
    return (s[2] == '\0' && setSingleCharOp.Contains(s[1])) ||
           testOperator.InList(s + 1);
  }

  static ILexer5 *LexerFactoryBash() { return new LexerBash(); }
};

Sci_Position SCI_METHOD LexerBash::PropertySet(const char *key,
                                               const char *val) {
  if (osBash.PropertySet(&options, key, val)) {
    if (strcmp(key, "lexer.bash.special.parameter") == 0) {
      setParamStart =
          LexillaCharacterSet(LexillaCharacterSet::setAlphaNum, "_");
      setParamStart.AddString(options.specialParameter.empty()
                                  ? BASH_SPECIAL_PARAMETER
                                  : options.specialParameter.c_str());
    }
    return 0;
  }
  return -1;
...
void SCI_METHOD LexerBash::Lex(Sci_PositionU startPos, Sci_Position length,
  ...
  LexillaCharacterSet setBashOperator(LexillaCharacterSet::setNone,
                                      "^&%()-+=|{}[]:;>,*/<?!.~@");
  LexillaCharacterSet setSingleCharOp(LexillaCharacterSet::setNone,
                                      "rwxoRWXOezsfdlpSbctugkTBMACahGLNn");
  LexillaCharacterSet setParam(LexillaCharacterSet::setAlphaNum, "_");
  LexillaCharacterSet setHereDoc(LexillaCharacterSet::setAlpha,
                                 "_\\-+!%*,./:?@[]^`{}~");
  LexillaCharacterSet setHereDoc2(LexillaCharacterSet::setAlphaNum,
                                  "_-+!%*,./:=?@[]^`{}~");
  ...
  while (sc.More()) {
    ...
    if (sc.atLineStart) {
      CmdState state =
          CmdState::Body; // force backtrack while retaining cmdState
      if (!StyleForceBacktrack(MaskCommand(sc.state))) {
        // retain last line's state
        // arithmetic expression and double bracket test can span multiline
        // without line continuation
        if (!QuoteStack.lineContinuation &&
            !AnyOf(cmdState, CmdState::DoubleBracket, CmdState::Arithmetic)) {
    ...
    case SCE_SH_OPERATOR:
      ...
      if (cmdState ==
          CmdState::Delimiter) // if command delimiter, start new command
    ...
    case SCE_SH_WORD:
      ...
      if (!setWord.Contains(sc.ch) || sc.Match('+', '=') ||
        ...
        // allow keywords ending in a whitespace, meta character or command
        // delimiter
        char s2[10]{};
        s2[0] = static_cast<char>(sc.ch);
        s2[1] = '\0';
        const bool keywordEnds = IsASpace(sc.ch) ||
                                 setMetaCharacter.Contains(sc.ch) ||
                                 cmdDelimiter.InList(s2);
        ...
        // disambiguate option items and file test operators
          ...
          if (!AnyOf(cmdState, CmdState::Test, CmdState::SingleBracket,
                     CmdState::DoubleBracket) ||
    ...
    case SCE_SH_HERE_DELIM:
      ...
      if (HereDoc.State == 0) { // '<<' encountered
        ...
        if (sc.chNext == '\'' ||
            sc.chNext == '\"') { // a quoted here-doc delimiter (' or ")
          sc.Forward();
          HereDoc.Quoted = true;
          HereDoc.State = 1;
        ...
        if (HereDoc.Quoted && sc.ch == HereDoc.Quote &&
            (HereDoc.BackslashCount & 1) ==
                0) { // closing quote => end of delimiter
          sc.ForwardSetState(SCE_SH_DEFAULT | insideCommand);
        } else if (sc.ch == '\\' && HereDoc.Quote != '\'') {
          HereDoc.Escaped = true;
          HereDoc.BackslashCount += 1;
          if ((HereDoc.BackslashCount & 1) == 0 ||
              (HereDoc.Quoted && !AnyOf(sc.chNext, '\"', '\\'))) {
            // in quoted prefixes only \ and the quote eat the escape
            HereDoc.Append(sc.ch);
          } else {
            // skip escape prefix
          }
        } else if (HereDoc.Quoted || setHereDoc2.Contains(sc.ch) ||
                   (sc.ch > 32 && sc.ch < 127 &&
                    (HereDoc.BackslashCount & 1) != 0)) {
          HereDoc.BackslashCount = 0;
          HereDoc.Append(sc.ch);
        } else {
          sc.SetState(SCE_SH_DEFAULT | insideCommand);
    ...
    case SCE_SH_HERE_Q:
      ...
      if (sc.atLineStart && QuoteStack.Current.Style == QuoteStyle::HereDoc) {
        ...
        if ((static_cast<Sci_Position>(
                 sc.currentPos + HereDoc.DelimiterLength) == sc.lineEnd) &&
    ...
    case SCE_SH_BACKTICKS:
      if (sc.ch == '\\') {
        ...
        if (QuoteStack.Current.Style == QuoteStyle::String ||
            ...
            QuoteStack.Current.Style ==
                QuoteStyle::LString) { // do nesting for "string",
                                       // $"locale-string", heredoc
          const bool stylingInside =
              options.stylingInside(MaskCommand(sc.state));
          if (sc.ch == '`') {
            ...
            continue;
                   ...
                   QuoteStack.Current.Style ==
                       QuoteStyle::Backtick) { // do nesting for $(command),
                                               // `command`, ${parameter}
          const bool stylingInside =
              options.stylingInside(MaskCommand(sc.state));
          if (sc.ch == '\'') {
    ...
    if (HereDoc.State == 1 && sc.MatchLineEnd()) {
      ...
      // first line of here-doc seem to follow the style of the last EOL
      // sequence
      HereDoc.State = 2;
      if (HereDoc.Quoted) {
    ...
    if (stylePrev != SCE_SH_DEFAULT &&
        MaskCommand(sc.state) == SCE_SH_DEFAULT) {
    ...
    if (MaskCommand(sc.state) == SCE_SH_DEFAULT) {
      if (sc.ch == '\\') {
        ...
        sc.SetState(((cmdState == CmdState::Arithmetic) ? SCE_SH_IDENTIFIER
                                                        : SCE_SH_WORD) |
                    insideCommand);
      } else if (sc.ch == '#') {
          ...
          if (sc.chPrev == '[') { // [#8] [##8] output digit setting
            ...
            if (sc.chNext == '#') {
              sc.Forward();
                     ...
                     IsUpperCase(sc.GetRelative(3))) { // ##^A
            sc.SetState(SCE_SH_IDENTIFIER | insideCommand);
            sc.Forward(3);
        ...
        if (sc.GetRelative(2) == '-') { // <<- indent case
          ...
          HereDoc.Indent = false;
                 ...
                 cmdState != CmdState::Arithmetic && sc.chPrev != '~' &&
                 !IsADigit(sc.chNext)) {
        ...
        if (QuoteStack.Current.Style == QuoteStyle::Arithmetic ||
            QuoteStack.Current.Style == QuoteStyle::CommandInside) {
...
void SCI_METHOD LexerBash::Fold(Sci_PositionU startPos_, Sci_Position length,
                                int initStyle, IDocument *pAccess) {
  if (!options.fold)
    return;
  ...
  for (Sci_Position i = startPos; i < endPos; i++) {
    ...
    if (atEOL) {
      const int lev =
          levelPrev | FoldLevelFlags(levelPrev, levelCurrent,
...
extern const LexerModule lmBash(SCLEX_BASH, LexerBash::LexerFactoryBash, "bash",
                                bashWordListDesc);
